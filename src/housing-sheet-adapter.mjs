...
export function sheetSettings(env){return{ready:Boolean((env.GOOGLE_SERVICE_ACCOUNT_JSON_BASE64||env.GOOGLE_SERVICE_ACCOUNT_JSON||env.GOOGLE_APPLICATION_CREDENTIALS)&&(env.HOUSING_SHEET_ID||env.SHEET_ID)&&env.HOUSING_SHEET_TAB),sheetId:env.HOUSING_SHEET_ID||env.SHEET_ID||null,tab:env.HOUSING_SHEET_TAB||null};}
...
export async function readHousingSheet({env,fetcher=fetch}){
 if(env.CRM_SYNTHETIC_SHEETS_ENABLED!=='true'&&env.CRM_HOUSING_SHEET_READ_ENABLED!=='true')throw Error('Sheets adapter disabled');
 const settings=sheetSettings(env);if(!settings.ready)throw Error('Sheet configuration incomplete');
 const account=decodeServiceAccount(env.GOOGLE_SERVICE_ACCOUNT_JSON_BASE64||env.GOOGLE_SERVICE_ACCOUNT_JSON||env.GOOGLE_APPLICATION_CREDENTIALS);
 const now=Math.floor(Date.now()/1000),header=base64url({alg:'RS256',typ:'JWT'}),claims=base64url({iss:account.client_email,scope:'https://www.googleapis.com/auth/spreadsheets.readonly',aud:account.token_uri,iat:now,exp:now+900}),input=header+'.'+claims;const signer=createSign('RSA-SHA256');signer.update(input);const jwt=input+'.'+signer.sign(account.private_key).toString('base64url');
 ...
 const range=encodeURIComponent("'"+settings.tab.replaceAll("'","''")+"'!A:AT");const url='https://sheets.googleapis.com/v4/spreadsheets/'+encodeURIComponent(settings.sheetId)+'/values/'+range;
