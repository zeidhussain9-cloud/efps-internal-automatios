...
export function sheetSettings(env){return{ready:Boolean(env.GOOGLE_SERVICE_ACCOUNT_JSON_BASE64&&env.HOUSING_SHEET_ID&&env.HOUSING_SHEET_TAB),sheetId:env.HOUSING_SHEET_ID||null,tab:env.HOUSING_SHEET_TAB||null};}
export function decodeServiceAccount(encoded){let json;try{json=JSON.parse(Buffer.from(encoded,'base64').toString('utf8'))}catch{throw Error('Invalid service-account JSON encoding')}if(json?.type!=='service_account'||!json.client_email||!json.private_key||!json.token_uri?.startsWith('https://oauth2.googleapis.com/'))throw Error('Invalid service-account JSON fields');return json;}
export async function readHousingSheet({env,fetcher=fetch}){
 ...
 const account=decodeServiceAccount(env.GOOGLE_SERVICE_ACCOUNT_JSON_BASE64);
