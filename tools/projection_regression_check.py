...
MODULE_ROOT=Path(__file__).resolve().parents[1]/"modules"/"efps-inventory-mgmnt";sys.path.insert(0,str(MODULE_ROOT));field_resolution=importlib.import_module("src.field_resolution");extract=importlib.import_module("src.extract");pipeline=importlib.import_module("src.pipeline")
CASES=[
...
("duplex_villa_subtype","Fully Furnished 4 BHK Duplex Villa",lambda r:r=="Duplex",field_resolution.resolve_property_subtype),
...
def main()->int:
    ...
    for name,source,predicate,fn in CASES:
        result=fn(source);ok=predicate(result);print(f"{name}: {'PASS' if ok else 'FAIL'} | {result!r}");failures += [] if ok else [name]
    integration_source="Semi Furnished 3 BHK with 3 Bathrooms & 2 Balconies\nPets: Not Allowed\nGated Community";case=pipeline.deterministic(integration_source);integration={"internal_property_type":case.get("internal_property_type")=="Gated Community","balconies":case.get("balconies")=="2","pet_friendly":case.get("pet_friendly")=="No","covered_parking":case.get("covered_parking")=="1","society_amenities":case.get("society_amenities")=="Club House, Lift, Gym, CCTV, Power Backup, Swimming Pool, Garden, Sports, Kids Area"}
    for name,ok in integration.items():print(f"integration_{name}: {'PASS' if ok else 'FAIL'} | {case.get(name)!r}");failures += [] if ok else [f"integration_{name}"]
    if failures:print("FAILED:",", ".join(failures));return 1
    print("ALL PROJECTION REGRESSIONS PASS");return 0
