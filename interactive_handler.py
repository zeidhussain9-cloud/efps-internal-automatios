...
def lambda_handler(event,context):
 ...
 try:
  ...
  reference=""
  try:reference=str((payload.get("actions") or [{}])[0].get("action_id") or payload.get("view",{}).get("callback_id") or "")
  except Exception:pass
  crash_report.report("slack_interactive",exc,reference=reference)
  return response({"response_type":"ephemeral","text":"EFPS could not apply that action. The error was logged for review."})
