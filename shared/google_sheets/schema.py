...
# Physical order is the latest verified user-supplied Housing_Listings order.
# Stage 2 is deliberately one business-processing stage; extraction,
# normalization, Maps, validation, AI verification and AI wording are sub-steps.
_ROWS = [
    ("listing_id", PANEL, STAGE_1, (), (), "Immutable row identity; EF-YYMM-XXXX."),
    ("status", PANEL, STAGE_1, ("Raw", "Pending", "Needs Review"), (), "Raw at creation; processing changes it to Pending or Needs Review."),
    ("intake_status", PANEL, STAGE_1, ("Raw", "Processed"), (), "Intake bookkeeping."),
    ("internal_property_type", PANEL, STAGE_2, ("Gated Community", "Semi Gated", "Standalone"), (), "Internal classification only."),
    ("listing_state", PANEL, STAGE_3, ("Available", "Rented Out", "On Hold"), (), "Lifecycle/downstream control; not populated by Phase 1."),
    ...
    ("furnish_type", PANEL, STAGE_2, ("Fully Furnished", "Semi Furnished", "Unfurnished"), (), "Explicit source value."),
    ...
    ("preferred_tenant_type", PANEL, STAGE_2, (), (), "Controlled sheet value; exact dropdown vocabulary is not verified in repository source."),
    ("bachelor_preference", PANEL, STAGE_2, (), ("preferred_tenant_type",), "Interdependent with preferred tenant type; exact dropdown vocabulary is not verified in repository source."),
    ("pet_friendly", PANEL, STAGE_2, (), (), "Source/verified value; exact dropdown vocabulary is not verified in repository source."),
    ...
    ("society_amenities", PANEL, STAGE_2, (), ("internal_property_type",), "Controlled amenity vocabulary; deterministic tier defaults apply only when blank."),
    ("flat_furnishings", PANEL, STAGE_2, (), ("furnish_type",), "Controlled furnishing vocabulary; defaults apply only when blank."),
    ("property_highlights", PANEL, STAGE_2, (), (), "AI wording/beautification only; no invented facts."),
    ...
    ("error_notes", HOUSING_AGENT, STAGE_3, (), (), "Housing Portal downstream-owned error/status notes."),
    ...
    ("inventory_locked", PANEL, STAGE_3, ("Yes", "No"), (), "Inventory lifecycle control; not populated by current Phase 1."),
]
...
# Current ownership is non-contiguous only at the Stage-3 boundary:
# panel owns A:AO and AU:AV; Housing owns AP:AR; Meta owns AS:AT.
...
def row_to_mapping(values: list[Any] | tuple[Any, ...]) -> dict[str, Any]:
    return dict(zip(NAMES, validate_row(values)))
...
def _check() -> None:
    ...
    assert letter("listing_id") == "A"
    assert letter("raw_message_text") == "G"
    assert letter("google_maps_url") == "L"
    assert letter("transaction_type") == "AM"
    assert letter("property_subtype") == "AN"
    assert letter("city") == "AO"
    assert letter("posted_url") == "AP"
    assert letter("error_notes") == "AR"
    assert letter("meta_catalog_id") == "AS"
    assert letter("meta_catalog_status") == "AT"
    assert letter("source_group") == "AU"
    assert letter("inventory_locked") == "AV"
    assert tuple(NAMES[:41]) == NAMES[:41]
    ...
    assert "source_group" in writable_by(PANEL) and "inventory_locked" in writable_by(PANEL)
