...
def resolve_signing_secret() -> str:
    ...
    if isinstance(value, dict):
        return str(value.get("signing_secret") or value.get("SLACK_SIGNING_SECRET") or value.get("value") or "").strip()
    return str(value).strip()

def verify_signature(raw_body: bytes, timestamp: str, signature: str, *, now: int | None = None, max_age_seconds: int = 300, signing_secret: str | None = None) -> bool:
