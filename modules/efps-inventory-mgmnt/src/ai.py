...
def _gemini(prompt):
    ...
    from shared.google_sheets import config as sheet_config
    info=sheet_config.service_account_info(); creds=service_account.Credentials.from_service_account_info(info,scopes=['https://www.googleapis.com/auth/cloud-platform']);creds.refresh(Request())
...
def review(raw,row,llm=None):
    facts={k:str(row.get(k,'')) for k in REVIEW_FIELDS if str(row.get(k,'' )).strip()}; prompt=('Compare CANONICAL FACTS to SOURCE. Return JSON {"conflicts":[{"field":"...","reason":"..."}]}. Report only clear contradictions; never missing optional fields or guesses.\nSOURCE:\n'+raw+'\nFACTS:\n'+json.dumps(facts,ensure_ascii=False))
    data=json.loads((llm or _gemini)(prompt)); conflicts=data.get('conflicts',[]);return [x for x in conflicts if isinstance(x,dict) and x.get('field') in REVIEW_FIELDS and str(x.get('reason','')).strip()]
...
def apply(row,raw,llm=None):
    ...
    if conflicts:
        out['status']='Needs Review';out['_ai_conflicts']=conflicts;return out
