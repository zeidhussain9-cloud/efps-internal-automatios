...
def resolve_maintenance(text: str) -> tuple[str,str]:
    candidates=[]
    for i,segment in enumerate(_segments(text)):
        # Labelled maintenance is the strongest evidence. Capture the whole
        # semantic expression so qualifiers such as '+ Water' are preserved.
        ...
        # Alternate source format: '50K + 6K Maintenance'. The second amount
        # is the maintenance amount; the first belongs to rent.
