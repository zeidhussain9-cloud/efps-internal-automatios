...
def _value(message,key,default=''):return getattr(message,key,default) if hasattr(message,key) else message.get(key,default)
...
def _persist_raw(client,session):
    ...
    client.write_range(schema.SHEET_ID,schema.WORKSHEET_NAME,schema.range_for('raw_message_text','intake_status',n),[[session.raw_text,'Raw']])
def handle(message,*,store=None,sheets_client=None,maps_client=None,ai_llm=None):
    store=store or DEFAULT_STORE;state,session=ingest(message,store)
    if state=='opened' and sheets_client is not None:
        session.listing_id=next_listing_id(sheets_client);session.source_group=str(_value(message,'chat_id'));store.put(session)
        write_new_property(sheets_client,initial_row(session.listing_id,'',session.source_group));return {'state':'opened','listing_id':session.listing_id}
    ...
    existing=schema.row_to_mapping(sheets_client.read_range(schema.SHEET_ID,schema.WORKSHEET_NAME,schema.full_range(n))[0]);existing['raw_message_text']=session.raw_text
    processed,issues=process_closed_session(session.raw_text,row=existing,maps_client=maps_client,ai_llm=ai_llm);write_phase1_update(sheets_client,n,processed)
