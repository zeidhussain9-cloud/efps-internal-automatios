...
_MAINT_NUMERIC = re.compile(r"^\s*[\d.,]+\s*(?:k|l|lakh|lakhs)?\.?\s*$", re.I)
...
def normalize_maintenance(value: str) -> str:
    ...
    if raw.lower() == "included":
        return "0"
    if _MAINT_NUMERIC.fullmatch(raw):
        m = re.fullmatch(r"\s*([\d.,]+)\s*(k|l|lakh|lakhs)?\.?\s*", raw, re.I)
        if m:
...
def apply_tenant_bachelor_rule(row: dict, raw_text: str) -> dict:
    ...
    if "family" in tenant and "female" in tenant and "bachelor" in tenant:
        row["preferred_tenant_type"] = "Open For All"
        row["bachelor_preference"] = "Female Only"
        return row
    if tenant in {"family", "family only"}:
        current = str(row.get("bachelor_preference", "")).strip()
        if not (current and _verified_in_text(current, raw_text)):
...
def normalize(row: dict, raw_text: str = "") -> dict:
    ...
    if out.get("maintenance"):
        out["maintenance"] = normalize_maintenance(out["maintenance"])
