...
def _report(row,issues,flags,trace):
    return {"populated_fields":[n for n in schema.NAMES if str(row.get(n,"") or "").strip()],"blank_fields":[n for n in schema.NAMES if not str(row.get(n,"") or "").strip()],"unresolved_fields":[n for n in ("internal_property_type","locality") if not str(row.get(n,"") or "").strip()],"review_flags":flags,"issues":list(issues),"trace":trace}
