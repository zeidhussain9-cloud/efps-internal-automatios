...
def safe_text(text: str) -> str:
    out=str(text or "").replace("<!channel>","@channel").replace("<!here>","@here").replace("<!everyone>","@everyone")
...
def buttons(lead:dict)->list[dict]:
    phone=lead.get("phone_number","");stage=lead.get("stage");action=lead.get("action"); items=[]
    ...
    chunks=[items[:5],items[5:]+extras[:2],extras[2:]]
    return [{"type":"actions","elements":chunk[:5]} for chunk in chunks if chunk]
def blocks(lead:dict)->list[dict]:
    ...
    title=name if name==shown else f"{name} · {shown}";unread=int(lead.get("unread_count") or 0)
    if unread:title+=f" ({unread} unread)"
    out=[{"type":"header","text":{"type":"plain_text","text":title[:150]}},{"type":"section","fields":[{"type":"mrkdwn","text":f"*Stage*\n{L.STAGE_EMOJI.get(stage,'')} {stage}"},{"type":"mrkdwn","text":f"*Action*\n{L.ACTION_EMOJI.get(action,'')} {action}"}],"accessory":{"type":"button","text":{"type":"plain_text","text":"💬 Open chat"},"url":chat_url(lead),"action_id":"chat::open"}},{"type":"section","text":{"type":"mrkdwn","text":f"*Looking for*\n{_requirement(lead)}"}}]
...
def post_or_update(lead:dict,*,channel:str="C0BTM6PH55L",client:SlackClient|None=None,_res=None)->str:
    ...
    ts=slack.post_message(channel=channel,text=payload["text"],blocks=payload["blocks"])
