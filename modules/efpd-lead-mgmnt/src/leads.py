...
NEW, CONTACTED, SHARED, VISITING, NEGOTIATING, BOOKED, CLOSED, LOST, ON_HOLD = (
    "New", "Contacted", "Shared", "Visiting", "Negotiating", "Booked", "Closed", "Lost", "On hold"
)
STAGES = [NEW, CONTACTED, SHARED, VISITING, NEGOTIATING, BOOKED, CLOSED, LOST, ON_HOLD]
TERMINAL = {CLOSED, LOST}
OUR_REPLY, THEIR_REPLY, NO_MATCH, VISIT_DUE, FOLLOWUP_DUE, NOTHING = (
    "Our reply", "Their reply", "No match", "Visit due", "Follow-up due", "None"
)
...
def display_phone(phone: str) -> str:
    ...
    if len(p) == 12 and p.startswith("91"): return f"+91 {p[2:7]} {p[7:]}"
    return "+" + p if p else "unknown"

def label(lead: dict) -> str:
    return str(lead.get("customer_name") or "").strip() or display_phone(lead.get("phone_number", ""))
...
def ist_label(ts: str) -> str:
    dt = _parse(ts)
    return dt.astimezone(IST).strftime("%-d %b, %-I:%M %p").lower() if dt else "—"
...
def quiet_days(lead: dict, *, _now=None) -> int:
    dt = _parse(lead.get("last_message_at", ""))
    return max(0, (((_now or datetime.now(timezone.utc)) - dt).days)) if dt else 0
...
def parse_followup(text: str, *, _today: date | None = None) -> str:
    ...
    if m:
        ...
        if m.group(2)[:3] in months:
            y = int(m.group(3)) if m.group(3) else today.year
            try: d = datetime(y, months.index(m.group(2)[:3]) + 1, int(m.group(1))).date()
            except ValueError: raise LeadError(f"no such date: {text}") from None
...
def next_action(lead: dict, *, _today: date | None = None) -> str:
    ...
    today = (_today or datetime.now(IST).date()).isoformat()
    if str(lead.get("next_followup_date") or "") and str(lead["next_followup_date"]) <= today: return FOLLOWUP_DUE
...
def next_stage(lead: dict, direction: str) -> str:
    ...
    return CONTACTED if direction == "out" and stage == NEW else stage
...
def record_message(phone: str, direction: str, body: str, *, message_id: str = "", sender_name: str = "", group_name: str = "", _res=None) -> dict:
    ...
    existing = db.get_lead(key, _res=_res) or {}
    created = not existing
    if message_id and db.interaction_exists(key, message_id, _res=_res):
        existing["_created"] = False; existing["_duplicate"] = True; return existing
    db.log_interaction(key, direction, body, message_id=message_id, group_name=group_name, _res=_res)
    lead = dict(existing)
    lead.update({"phone_number": key, "last_message": str(body or "")[:400], "last_message_at": now_iso(), "last_direction": direction})
    ...
    stored = db.put_lead(lead, _res=_res); stored["_created"] = created; stored["_duplicate"] = False
    return stored
...
def set_fields(phone: str, updates: dict, *, _res=None) -> dict:
    allowed = {"customer_name","looking_requirement","budget_min","budget_max","preferred_localities","interested_listings","lost_reason"}
    lead = db.get_lead(normalise_phone(phone), _res=_res)
    if not lead: raise LeadError("lead not found")
...
def refresh_actions(*, _res=None, _today=None) -> list[dict]:
    ...
    for lead in db.all_leads(_res=_res):
        ...
        if wanted != lead.get("action"):
            lead["action"] = wanted; changed.append(db.put_lead(lead, _res=_res))
    return changed

def queue(rows: list[dict], *, _now=None) -> list[dict]:
    priority = {OUR_REPLY:0, FOLLOWUP_DUE:1, VISIT_DUE:2, NO_MATCH:3, THEIR_REPLY:4, NOTHING:5}
    return sorted([r for r in rows if r.get("stage") not in TERMINAL], key=lambda r:(priority.get(r.get("action"),9), str(r.get("last_message_at") or "")))
