...
def _clip(value:str)->str:
    text=str(value or "");return text if len(text)<=MAX_VALUE else text[:MAX_VALUE]+"…"
def diff(before:dict|None,after:dict)->list[dict]:
    ...
    for field in TRACKED:
        ...
        if old!=new:out.append({"field":field,"old":_clip(old),"new":_clip(new)})
    return out
def record(phone:str,changes:list[dict],*,_res=None)->list[dict]:
    ...
    for change in changes:
        item={"phone_number":str(phone),"changed_at":f"{stamp}#{uuid.uuid4().hex[:6]}","field":change["field"],"old_value":change["old"],"new_value":change["new"],"source":source,"actor":who}
        if reason:item["reason"]=reason
        try:db._table(db.DDB_AUDIT,_res).put_item(Item=item);written.append(item)
        except Exception as exc:print(f"lead audit write failed for {phone}: {exc!r}")
    return written
def history(phone:str,limit:int=20,*,_res=None)->list[dict]:
    try:return db._table(db.DDB_AUDIT,_res).query(KeyConditionExpression="phone_number = :p",ExpressionAttributeValues={":p":str(phone)},ScanIndexForward=False,Limit=limit).get("Items",[])
    except Exception as exc:print(f"lead audit read failed: {exc!r}");return []
def describe(entry:dict)->str:
    when=str(entry.get("changed_at","")).split("#")[0]
    try:when=datetime.fromisoformat(when).astimezone(timezone.utc).strftime("%-d %b %-I:%M %p")
